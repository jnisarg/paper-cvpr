...
def val2list(x: Union[list, tuple], repeat_time=1) -> list:
    if isinstance(x, (list, tuple)):
        return list(x)
    return [x for _ in range(repeat_time)]


def val2tuple(x: Union[list, tuple], min_len: int = 1, idx_repeat: int = -1) -> tuple:
    x = val2list(x)
...
def squeeze_list(x: list | None) -> Union[list, None]:
